...
?>
<button onClick="javascript:window.open('setsubkindshow_print.php','_blank')">列印班級總報表</button>
<?php
 //取得教師所上年級、班級
      ...
      if($row_count<>0)  {
       //將student_sn轉成陣列字串
         $select_sn='';
           ...
           /*
           echo $count_stud_class_id."<br>";
           print_r($stud_class_id);
           echo "<br>";
           for ($ii=0;$ii<=$count_stud_class_id;$ii++) {
			   echo $ii."<br>"; 
			  echo  $stud_class_id[$ii]."<br>";  
			}
			*/ 
      ...
      //是否開放導師可以自由輸入屬性
       ...
       if($class_num) {  if($m_arr['free_input']='Y') $free_input=''; else $free_input='disabled'; }
                   ...
                   for($ki=0;$ki< $count_stud_class_id;$ki++) {
					    $listdata.="<table width='100%' border=1 cellspacing='1' cellpadding='1' bordercolor='#CCCCFF'>
                                <tr bgcolor='#CCCCFF'>
                                  <td>NO.</td>
                                  <td>年班座號</td>
                                  <td>學號</td>
